...
Section.prototype.extractHeading = function() {
  ...
  if (this.block.is('.QOA')) {
    this.section.headings.push(this.block.children('h2').first().text().trim());
  } else {
...
Section.prototype.extractEvents = function() {
  var blocks = this.block.children('div:not(.section), ul, p.Speech, p.SubsQuestion, table.vote');
  ...
  this.section.events = _.map(blocks, function(block) {
    ...
    if (block.is('p.Speech, p.SubsQuestion, table.vote')) {
